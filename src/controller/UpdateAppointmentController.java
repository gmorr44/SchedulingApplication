...
public class UpdateAppointmentController implements Initializable {
    ...
    void saveBTN(ActionEvent event) throws IOException, SQLException{
        ...
        try{
            ...
            if(Time.openDateChecks(apptDate) == false ){//Checks to see if the date is during business hours.
                if(Time.appointmenInversionCheck(startLT, endLT) == false){//checks for inverted start and end times.
                    if((Time.appointmentConflict(apptDate, startLT, endLT,user,appt,customer)== false) ){//checks for any other appointment conflicts.
    
                    // Creating a new appointment into the database.
                    AppointmentDAO.updateAppointment(customer,user,appt, type,start, end);//executes the update to the database.
        ...
        catch(NullPointerException e){//catches the exception if the date picker field is empty.
            
            Alert alert = new Alert(Alert.AlertType.WARNING);
            alert.setTitle("Error Dialog");
            alert.setContentText("Please complete all fields before saving updating appointment");
            alert.showAndWait();    
        }
    }
     ...
     public void sendAppointment(Appointment appointment){
         ...
         //The sendAppointment method retrieves the data from the last scene to use in the text fields.
        String ds = appointment.getDate();
        String st = appointment.getStart();
        String et = appointment.getEnd();
        LocalDate ld = Time.stringToDate(ds,st);
        LocalTime ltStart = Time.stringToTime(ds, st);
        LocalTime ltend = Time.stringToTime(ds, et);
        
        appt = appointment.getAppointmentId();
        ...
        apptType.getSelectionModel().select(appointment.getType());
        
        date.setValue(ld);
        startTime.setValue(ltStart);
        endTime.setValue(ltend);
    }
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        try{
            ...
            //The following loop populates the combo boxes for time selection.
            ...
            LocalTime start1 = LocalTime.of(9, 0);//Regular business hours 0900-1700.
